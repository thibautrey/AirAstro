...
export class EquipmentManagerService extends EventEmitter {
  ...
  async performAutoSetup(): Promise<AutoSetupResult> {
    ...
    try {
      ...
      for (const device of devices) {
        if (device.autoInstallable && device.driverStatus !== 'running') {
          ...
          try {
            ...
            result.errors.push(`Erreur lors de la configuration de ${device.name}: ${status.errorMessage}`);
            
            this.equipmentStatus.set(device.id, status);
            this.emit('equipmentStatusChanged', status, device);
            
            console.error(`❌ Erreur lors de la configuration de ${device.name}:`, error);
          }
        }
      ...
      console.log(`🎉 Configuration automatique terminée: ${result.configured} configurés, ${result.failed} échecs`);
  ...
  async setupSingleDevice(deviceId: string): Promise<boolean> {
    ...
    if (!device.autoInstallable) {
      throw new Error(`Équipement ${device.name} ne peut pas être configuré automatiquement`);
    }
