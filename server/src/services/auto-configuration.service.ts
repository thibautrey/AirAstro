...
export class AutoConfigurationService {
  ...
  constructor() {
    this.configScriptPath = path.join(
      __dirname,
      "../scripts/auto-configure-equipment.sh"
    );
  }
  ...
  async runAutoConfiguration(): Promise<AutoConfigReport> {
    ...
    try {
      console.log(
        "🔧 Démarrage de la configuration automatique des équipements..."
      );
      ...
      try {
        ...
        throw new Error(
          `Script de configuration non trouvé: ${this.configScriptPath}`
        );
      }
      ...
      console.log(
        `📊 Rapport: ${report.drivers_installed} drivers installés, ${report.usb_devices_detected} appareils détectés`
      );
  ...
  async readConfigReport(): Promise<AutoConfigReport> {
    try {
      ...
      console.warn(
        "⚠️ Impossible de lire le rapport de configuration, création d'un rapport par défaut"
      );
  ...
  private async getServiceStatus(serviceName: string): Promise<string> {
    try {
      const { stdout } = await exec(
        `systemctl is-active ${serviceName}.service 2>/dev/null`
      );
      return stdout.trim();
    } catch (error) {
      return "inactive";
  ...
  async installDriversForDevices(
    deviceList: string[]
    ...
    console.log(
      `🔧 Installation de drivers pour ${deviceList.length} appareils...`
    );
    ...
    for (const device of deviceList) {
      try {
        ...
        failed.push(device);
        console.error(
          `❌ Erreur lors de l'installation du driver pour ${device}:`,
          error
        );
      }
    }
  ...
  private mapDeviceToDriver(device: string): string | null {
    ...
    if (deviceLower.includes("zwo") || deviceLower.includes("asi")) {
      ...
      deviceLower.includes("skywatcher") ||
      deviceLower.includes("sky-watcher")
    ) {
      return "indi-eqmod";
    } else if (deviceLower.includes("camera") || deviceLower.includes("cam")) {
      return "indi-gphoto";
    } else if (
      deviceLower.includes("mount") ||
      deviceLower.includes("telescope")
    ) {
      return "indi-eqmod";
    }
  ...
  private async installSingleDriver(driverName: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `❌ Erreur lors de l'installation du driver ${driverName}:`,
        error
      );
      throw error;
    }
