...
# Script d'installation pour les appareils photo Canon DSLR via GPhoto2
...
if [ -f "$PROJECT_ROOT/scripts/core/airastro-common.sh" ]; then
    ...
    log_info() { echo -e "\033[1;32m[Canon Install]\033[0m $*"; }
    log_error() { echo -e "\033[1;31m[Error]\033[0m $*" >&2; }
    log_warning() { echo -e "\033[1;33m[Warning]\033[0m $*"; }
fi

log_info "🚀 Installation du support Canon DSLR"

# Vérifier les privilèges
if [ "$EUID" -eq 0 ]; then
    log_warning "Ce script ne doit pas être exécuté en tant que root"
    log_info "Utilisez: ./install-canon.sh"
    exit 1
fi
...
log_info "1. Diagnostic initial - détection des appareils Canon"
if lsusb | grep -q "04a9"; then
    ...
    log_info "Vérifiez que votre appareil Canon est connecté et allumé"
fi

# 2. Installation des drivers GPhoto2 et INDI
...
if [ -f "/lib/udev/rules.d/40-gphoto.rules" ] || [ -f "/etc/udev/rules.d/40-gphoto.rules" ]; then
    log_info "✅ Règles udev GPhoto2 déjà présentes"
else
    log_info "Création des règles udev pour Canon..."
    sudo bash -c 'cat > /etc/udev/rules.d/40-gphoto.rules << EOF
# Canon DSLR cameras
SUBSYSTEMS=="usb", ATTRS{idVendor}=="04a9", GROUP="users", MODE="0666"
# Alternative pour certains modèles Canon
SUBSYSTEMS=="usb", ATTRS{idVendor}=="04a9", ATTRS{idProduct}=="*", GROUP="plugdev", MODE="0664"
EOF'
    ...
    log_info "✅ Règles udev Canon configurées"
fi
...
if lsusb | grep -q "04a9"; then
    log_info "7. Test de connexion Canon"
    log_info "Pour tester votre appareil Canon, utilisez:"
    ...
    log_info "7. Connectez votre appareil Canon pour effectuer un test"
fi

log_info "✅ Installation Canon terminée"
log_info ""
log_info "📋 Résumé:"
...
log_info "   - Appareils Canon détectés: $(lsusb | grep -c "04a9" || echo "0")"
log_info ""
log_info "ℹ️  Instructions importantes:"
log_info "   1. Allumez votre appareil Canon"
...
log_info "   $SCRIPT_DIR/diagnose-canon.sh"
